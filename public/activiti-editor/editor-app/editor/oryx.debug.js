...
ORYX.Core.StencilSet.StencilSet = Clazz.extend({
    ...
    construct: function(source, modelMetaData, editorId){
      ...
      var headers = {}
      var token = localStorage.getItem(ACTIVITI.CONFIG.token)
      token = token && token.replace(/^"|"$/g, '')
      headers[ACTIVITI.CONFIG.token] = token
			new Ajax.Request(ACTIVITI.CONFIG.contextRoot + '/auth/activiti/model/stencilset?version=' + Date.now(), {
	            asynchronous: false,
	            method: 'get',
