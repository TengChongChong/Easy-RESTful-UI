        ...
        <div class="tree-wrapper">
          ...
          <a-dropdown :trigger="['contextmenu']" :visible="rightMenuVisible">
            <tree
              ...
              show-icon
              :draggable="$permissions(SYS_PERMISSIONS_CODE.SYS_ROLE_MOVE)"
              show-line
              @drop="onDrop"
              @select="onSelect"
              @expand="onExpand"
            ...
            <a-menu slot="overlay" @click="onRightMenuClick">
              <a-menu-item key="add" v-if="$permissions(SYS_PERMISSIONS_CODE.SYS_ROLE_SAVE)"><a-icon type="plus"/> 新增下级</a-menu-item>
              <a-menu-item key="edit" v-if="$permissions(SYS_PERMISSIONS_CODE.SYS_ROLE_SAVE)" :disabled="!rightMenuState.edit"><a-icon type="edit"/> 修改</a-menu-item>
              <a-menu-item key="remove" v-if="$permissions(SYS_PERMISSIONS_CODE.SYS_ROLE_REMOVE)" :disabled="!rightMenuState.remove"><a-icon type="delete"/> 删除</a-menu-item>
              <a-menu-item key="enable" v-if="$permissions(SYS_PERMISSIONS_CODE.SYS_ROLE_SAVE)" :disabled="!rightMenuState.enable"><a-icon type="check"/> 启用</a-menu-item>
              <a-menu-item key="disable" v-if="$permissions(SYS_PERMISSIONS_CODE.SYS_ROLE_SAVE)" :disabled="!rightMenuState.disable"><a-icon type="stop"/> 禁用</a-menu-item>
            </a-menu>
          </a-dropdown>
          ...
          <a-row class="form-row" :gutter="16">
            ...
            <a-col :sm="24">
              <div class="input-btn-group">
                <e-btn-save :permissions="SYS_PERMISSIONS_CODE.SYS_ROLE_SAVE" :loading="saveLoading" :click-callback="save"/>
                <a-popconfirm
                  v-if="$permissions(SYS_PERMISSIONS_CODE.SYS_ROLE_REMOVE)"
                  title="确定要删除吗?"
                ...
                <a-button type="default" icon="plus" @click="add(model.id)" v-if="$permissions(SYS_PERMISSIONS_CODE.SYS_ROLE_SAVE) && model.id != null && model.id !== ''">
                  新增下级
                </a-button>
